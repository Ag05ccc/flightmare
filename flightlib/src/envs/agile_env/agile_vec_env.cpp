...
AgileVecEnv<EnvBaseName>::AgileVecEnv() {
...
template<typename EnvBaseName>
AgileVecEnv<EnvBaseName>::AgileVecEnv(const std::string& cfg,
...
template<typename EnvBaseName>
AgileVecEnv<EnvBaseName>::AgileVecEnv(const YAML::Node& cfg_node) {
  cfg_ = cfg_node;
  configEnv(cfg_);
  random_reset_ = true;
}
...
template<typename EnvBaseName>
bool AgileVecEnv<EnvBaseName>::reset(Ref<MatrixRowMajor<>> obs) {
...
template<typename EnvBaseName>
bool AgileVecEnv<EnvBaseName>::reset(Ref<MatrixRowMajor<>> obs, bool random) {
  if (obs.rows() != this->num_envs_ || obs.cols() != this->obs_dim_) {
    this->logger_.error(
      "Input matrix dimensions do not match with that of the environment.");
    return false;
...
template<typename EnvBaseName>
bool AgileVecEnv<EnvBaseName>::step(Ref<MatrixRowMajor<>> act,
...
template<typename EnvBaseName>
void AgileVecEnv<EnvBaseName>::perAgentStep(int agent_id,
...
template<typename EnvBaseName>
bool AgileVecEnv<EnvBaseName>::getQuadAct(Ref<MatrixRowMajor<>> quadact) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
...
template<typename EnvBaseName>
bool AgileVecEnv<EnvBaseName>::getQuadState(Ref<MatrixRowMajor<>> quadstate) {
  bool valid = true;
  for (int i = 0; i < this->num_envs_; i++) {
...
template class AgileVecEnv<AgileEnv>;
